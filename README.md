...
**The Problem:** Knowledge workers collect information from everywhere — PDFs, audio recordings, articles, meeting notes. But searching through it later is painful. Most tools treat everything as static files instead of living knowledge.

**The Solution:** Timeline Thinker organizes content along a timeline and provides intelligent retrieval through:

- **⏰ Temporal-first retrieval** – Questions like "What did I learn last week about RAG?" are anchored to the right moment
- **📚 Multi-source ingestion** – PDFs, audio files, webpages, and text documents
- **🎯 Source-focused queries** – Select any source to focus your conversation on that specific content
- **🤖 Multi-agent pipeline** – Five specialized AI agents collaborate for accurate retrieval
- **📊 Session management** – Track today's active sources and export daily notes as PDF
- **🔍 Hybrid ranking** – Combines semantic similarity (60%), temporal relevance (20%), and entity overlap (20%)
...
- **Hybrid ranking** that outperforms pure semantic search (68% → 85% accuracy)
...
│  │  ┌─────────┐  ┌─────────┐  ┌──────────┐  ┌────────┐ │  │
│  │  │ Planner │→ │Timeline │→ │ Document │→ │Response│ │  │
...
┌─────────────────────────────────────────────────────────────┐
│              PostgreSQL + pgvector                           │
│   Events | Embeddings | Entities | Topics | Sessions        │
└─────────────────────────────────────────────────────────────┘
```
...
├── backend/
│   ├── app/
│   │   ├── agents/           # Multi-agent retrieval pipeline
│   │   │   ├── planner.py
...
│   │   ├── services/         # LLM, embeddings, sessions
